...
- (void)closeDB{
    ...
    if (!self.inTransaction && _queue) {//没有事务的情况下就关闭数据库.
        [_queue close];//关闭数据库.
        _queue = nil;
    }
...
/**
 创建表(如果存在则不创建).
 */
- (void)createTableWithTableName:(NSString * _Nonnull)name
                ...
                unionPrimaryKeys:(NSArray * _Nullable)unionPrimaryKeys
                      uniqueKeys:(NSArray * _Nullable)uniqueKeys
                        complete:(bg_complete_B)complete{
    NSAssert(name, @"表名不能为空!");
        ...
        for (int i = 0; i < keys.count; i++) {
            ...
            if (tempUniqueKeys.count && [tempUniqueKeys containsObject:key]) {
                ...
                if ([key isEqualToString:bg_primaryKey] && !unionPrimaryKeys.count) {
                    [sql appendFormat:@"%@ primary key", [BGTool keyAndType:keys[i]]];
                } else {
...
- (void)insertIntoTableName:(NSString * _Nonnull)name Dict:(NSDictionary * _Nonnull)dict complete:(bg_complete_B)complete {
    [self insertIntoTableName:name Dict:dict primaryKey:nil complete:complete];
}

/**
 插入数据.
 */
- (void)insertIntoTableName:(NSString * _Nonnull)name Dict:(NSDictionary * _Nonnull)dict primaryKey:(NSString *)primaryKey complete:(bg_complete_B)complete {
    NSAssert(name, @"表名不能为空!");
...
- (NSString *)getMainKeyVaule {
    NSString *value = [[NSUUID UUID] UUIDString] ? : [NSString stringWithFormat:@"%lld_%ld", (long long)([[NSDate date] timeIntervalSince1970] * 1000), random() % 10000];
    return value;
}
